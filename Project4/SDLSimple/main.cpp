...
#define FONT_COUNT_SUCC 7
#define FONT_COUNT_FAIL 9

struct GameState {
	Entity player;
	Entity platforms[PLATFORM_COUNT];
	Entity enemies[ENEMY_COUNT];
	Entity fontSucc[FONT_COUNT_SUCC];
	Entity fontFail[FONT_COUNT_FAIL];
};
...
void Initialize() {
	...
	int indices_successful[FONT_COUNT_SUCC] = {89, 79, 85, 9, 87, 73, 78};
	...
	for (int i = 0; i < FONT_COUNT_SUCC; i++) {
		...
		state.fontSucc[i].isActive = false;
	}

	int indices_failure[FONT_COUNT_FAIL] = { 71, 65, 77, 69, 9, 79, 86, 69, 82};
	for (int i = 0; i < FONT_COUNT_FAIL; i++) {
		state.fontFail[i].animIndex = indices_failure[i];
		state.fontFail[i].textureID = fontTextureID;
		state.fontFail[i].position = glm::vec3(i - 4, 1, 0);
		state.fontFail[i].isActive = false;
	}
...
void Render() {
	...
	if (state.player.isActive == false) {
		for (int i = 0; i < FONT_COUNT_FAIL; i++) {
			state.fontFail[i].RenderFont(&program);
		}
	}

	if (state.player.isActive == true) {
		if (state.enemies[0].isActive == false) {
			if (state.enemies[1].isActive == false) {
				if (state.enemies[2].isActive == false) {
					for (int i = 0; i < FONT_COUNT_SUCC; i++) {
						state.fontSucc[i].RenderFont(&program);
					}
				}
			}
		}
